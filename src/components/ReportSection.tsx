...
export const ReportSection = ({ number, title, subtitle, description, checklist, checklistLink, completion, completed, extraContent }: Props) => (
    ...
    <div className="flex items-start gap-3">
      ...
      <div className="space-y-2 flex-1">
        ...
        {completion !== undefined && (
          <div className="space-y-1">
            <div className="flex justify-between text-xs">
              ...
              <span className="text-tertiary font-mono font-bold">{completion}%</span>
            </div>
            <div className="w-full h-2 bg-muted rounded-full overflow-hidden">
              <div className="h-full bg-tertiary rounded-full transition-all" style={{ width: `${completion}%` }} />
            </div>
          </div>
        )}
        {extraContent}
        <div className="flex items-center gap-3">
          ...
          {checklistLink && (
            <a href={checklistLink} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-xs text-tertiary hover:underline">
